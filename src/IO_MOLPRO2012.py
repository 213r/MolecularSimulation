...
class InputMOLPRO:
    ...
    def set_cpmcscf_istate(self,now_state,nrange,q_nacme):
    ...
    def set_caspt2_istate(self,now_state,nrange):
        ...
        str_method = self.method_save
        str_method = str_method.replace("<nrange>", str(nrange)) 
        str_method = str_method.replace("<force>", "force") 
        self.method = str_method.replace("<istate>", str(now_state+1)) 
...
#        return self.get_potential_energy__mcscf_multi(nrange=1) 
    ...
    def get_ci_coeff_casscf(self):
        ...
        while l:
            if "CI vector" in l:
                for _ in xrange(3): l = f.readline()
                while l != "\n":
                    txt += l 
                    l = f.readline()
                break 
